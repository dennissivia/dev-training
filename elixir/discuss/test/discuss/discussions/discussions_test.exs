defmodule Discuss.DiscussionsTest do
  ...
  describe "topics" do
    alias Discuss.Discussions.Topics
    ...
    test "list_topics/0 returns all topics" do
      ...
      assert Discussions.list_topics() == [topics]
    end

    test "get_topics!/1 returns the topics with given id" do
      topics = topics_fixture()
      assert Discussions.get_topics!(topics.id) == topics
    end

    test "create_topics/1 with valid data creates a topics" do
      assert {:ok, %Topics{} = topics} = Discussions.create_topics(@valid_attrs)
      assert topics.title == "some title"
    end

    test "create_topics/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Discussions.create_topics(@invalid_attrs)
    end

    test "update_topics/2 with valid data updates the topics" do
      topics = topics_fixture()
      assert {:ok, topics} = Discussions.update_topics(topics, @update_attrs)
    ...
    test "update_topics/2 with invalid data returns error changeset" do
      topics = topics_fixture()
      assert {:error, %Ecto.Changeset{}} = Discussions.update_topics(topics, @invalid_attrs)
      assert topics == Discussions.get_topics!(topics.id)
    end

    test "delete_topics/1 deletes the topics" do
      topics = topics_fixture()
      assert {:ok, %Topics{}} = Discussions.delete_topics(topics)
      assert_raise Ecto.NoResultsError, fn -> Discussions.get_topics!(topics.id) end
    end

    test "change_topics/1 returns a topics changeset" do
      topics = topics_fixture()
      assert %Ecto.Changeset{} = Discussions.change_topics(topics)
    end
  end
