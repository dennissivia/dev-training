...
def read_edges(n, use_smallest: true, directed: false)
  edges = Array.new { [] }
  weights = []
  n.times do
    ...
    next if (use_smallest) && weights[u] && weights[u][v] && weights[u][v] < w
    add_edge(edges, weights, u, v, w)

    unless directed
      next if (use_smallest) && weights[v] && weights[v][u] && weights[v][u] < w

      add_edge(edges, weights, v, u, w)
    end
  end
...
def process_floyd
  ...
  nodes = (1..n).to_a
  edges, weights = read_edges(m, use_smallest: false, directed: true)

  adj = Graph.build_adjacency_list(nodes, edges, directed: true)

  q = gets.chomp.to_i
  queries = q.times.to_a.map do
    gets.chomp.split.map(&:to_i)
  end

  distances = Graph.dijkstra_multi_query(queries, adj, weights)
  print_distances(distances, delim: "\n")
end
process_floyd
